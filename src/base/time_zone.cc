...
class File: public netlib::NonCopyable
{
public:
...
struct tm TimeZone::toUtcTime(time_t secondsSinceEpoch, bool yday)
{
	...
	if (yday)
	{
		Date startOfYear(ymd.year_, 1, 1);
		utc.tm_yday = date.get_julian_day_number() - startOfYear.get_julian_day_number();
	}
	return utc;
}
                             ...
                             int hour, int minute, int seconds)
{
	...
	time_t days = date.get_julian_day_number() - Date::kJulianDayOf1970_01_01;
